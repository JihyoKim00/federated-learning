...
class NumPyClient(ABC):
    ...
    ) -> Tuple[List[np.ndarray], int, Dict[str, Scalar]]: # int
...
class NumPyClientWrapper(Client):
    ...
    def fit(self, ins: FitIns) -> FitRes:
        ...
        # Train
        results: Tuple[List[np.ndarray], int, Metrics] = self.numpy_client.fit( # int
        ...
        if not (
            ...
            # and isinstance(results[3], int)
        ):
            raise Exception(EXCEPTION_MESSAGE_WRONG_RETURN_TYPE_FIT)

        # Return FitRes
        parameters_prime, num_examples, metrics = results # loss
