...
def _init_defaults(
    ...
    if server is None:
        ...
        msg = f"Flower strategy is {strategy}"    
        log(INFO, msg)
    ...
    msg = f"Communication Round(global round) is {config['num_rounds']}"
    msg_2 = f"Fraction rate is {config['fraction_rate']}"
    msg_3 = f"Server GPU number is {config['gpu number']}"
