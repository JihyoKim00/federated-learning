...
class Server:
    ...
    def fit(self, num_rounds: int) -> History:
        ...
        # Evaluation Initialize parameters 
        ...
        log(INFO, "Federated Learning starting!")
        start_time = timeit.default_timer()

        for current_round in range(1, num_rounds + 1):
            
            # Print Communication round 
            msg = f"Communication round is going.... round is {current_round}"
            log(INFO, msg)
            ...
            log(INFO,
                "Transfering aggregated weight to global model...")
            ...
            if res_cen is not None:
                ...
                log(
                    INFO,
                    "Server-side evaluation : (%s, %s, %s, %s)",
                    current_round,
                    loss_cen,
                    metrics_cen,
    ...
    def fit_round(
        ...
        # Get clients and their respective instructions from strategy
        client_instructions, client_dict, client_num = self.strategy.configure_fit(
            rnd=rnd, parameters=self.parameters, client_manager=self._client_manager
        )
        ...
        log(
            DEBUG,
            "fit_round: strategy sampled %s clients (all client is %s)",
            len(client_instructions),
            self._client_manager.num_available(),
        )
        ...
        ] = self.strategy.aggregate_fit(rnd, results, client_dict, failures, client_num)
    ...
    def _get_initial_parameters(self) -> Parameters:
        ...
        # Server-side에서 parameter initilaization 진행
        ...
        if parameters is not None:
            log(INFO, "Using initial parameters provided by strategy(Server-side)")
            return parameters
