...
def get_dataloader(
    path_to_data: str, cid: str, is_train: bool, batch_size: int, workers: int, dataset='str'):
    """Generates trainset/valset object and returns appropiate dataloader."""

    partition = "train" if is_train else "val"

    if dataset == 'cifar':
        dataset = get_dataset(Path(path_to_data), cid, partition, cifar10Transformation())
        
    elif dataset == 'mnist':
        dataset = get_dataset(Path(path_to_data), cid, partition, MnistTransformation())
...
def getCIFAR10(path_to_data="./data"):
    ...
    train_set = datasets.CIFAR10(root=path_to_data, 
                                 train=True, 
                                 download=True, transform=cifar10Transformation())
...
def getMNIST(path_to_data="./mnist_data") : 

    train_set = datasets.MNIST(root=path_to_data, 
                               train=True, 
                               download=True, transform=MnistTransformation()) 
    
    data_loc = os.path.join(path_to_data,'processed')
    training_data = Path(path_to_data) / 'training.pt'
